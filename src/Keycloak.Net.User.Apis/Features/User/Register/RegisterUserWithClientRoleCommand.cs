...
internal class RegisterUserWithClientRoleCommand(IGetClientTokenQuery getClientTokenQuery, IGetClientIdQuery getClientIdQuery, IAssignRealmRoleInternalCommand userRoleCommand, IGetClientRoleQuery getClientRoleQuery, IGetUserIdQuery userRequest, IHttpClientFactory httpClientFactory, IOptionsMonitor<Server> server, IOptionsMonitor<AdminClient> adminClient) : IRegisterUserWithClientRoleCommand
{
    ...
    public async Task<Result> RegisterUserWithClientRole(string clientId, string username, string password, string email, string firstName, string lastName, string[] rolesName, CancellationToken cancellationToken = default)
    {
        ...
        var client = new GetClientTokenRequest(_adminClient.ClientSecret, _adminClient.ClientSecret);
        ...
        var result = await _userRoleCommand.AssignRealmRolesAsync(adminUrl, userResponse.Content!, roles.ToArray(), httpClient);
        if (!result.IsSuccess)
